...
def run_experiment(n_state, n_action, SASR, pi0, pi1, gamma):
    ...
    return est_DENR, est_naive_average, est_IST, est_ISS, est_WIST, est_WISS, est_model_based,est_model_double,est_model_based_mis_q, est_model_double_mis_q,est_DENR_mis_w,est_model_double_mis_w  

####if __name__ == '__main__':

estimator_name = ['On Policy', 'Density Ratio', 'Naive Average', 'IST', 'ISS', 'WIST', 'WISS', 'Model Based','double',"Model Base misq","Double misq","DR wmis","Double misq"]
length = 5
env = taxi(length)
n_state = env.n_state
...
res = np.zeros(100, dtype = np.float32)
for k in range(100):
    print k
    np.random.seed(k)
    ...
    res[k] = on_policy(np.array(SASR), gm)

    np.save('result/on_policy.npy', res)
